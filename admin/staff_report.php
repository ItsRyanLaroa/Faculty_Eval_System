...
<script>
	$(document).ready(function(){
		$('#staff_id').change(function(){
			if($(this).val() > 0)
			window.history.pushState({}, null, './index.php?page=staff_report&sid='+$(this).val());
			load_class()
		})
		if($('#staff_id').val() > 0)
			load_class()
	...
	function load_class(){
		...
		$.ajax({
			url:"ajax.php?action=get_staff_class",
			method:'POST',
	...
	function anchor_func(){
		$('.show-result').click(function(){
			...
			window.history.pushState({}, null, './index.php?page=staff_report&sid='+vars.sid+'&rid='+data.id);
			load_report(vars.sid,data.sid,data.id);
	...
	function load_report($staff_id, $subject_id,$class_id){
		if($('#preloader2').length <= 0)
		start_load()
		$.ajax({
			url:'ajax.php?action=get_staff_report',
			method:"POST",
			data:{staff_id: $staff_id,subject_id:$subject_id,class_id:$class_id},
			error:function(err){
				console.log(err)
				alert_toast("An Error Occured.","error");
				end_load()
