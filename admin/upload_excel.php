...
if (isset($_FILES['excel_file'])) {
    ...
    if ($_FILES['excel_file']['error'] == UPLOAD_ERR_OK) {
        ...
        foreach ($sheetData as $row) {
            ...
            $firstname = $row['B'];
            $lastname = $row['C'];
            $class_id = $row['D']; // Match column indices with your data
            ...
            // Prepare and bind (updated to include 6 variables)
        ...
        header('Location: ../index.php?page=student_list&success=1');
        exit;
    } else {
        // Handle file upload error
        header('Location: ../index.php?page=student_list&error=upload_error');
        exit;
    }
} else {
    // Handle missing file
    header('Location: ../index.php?page=student_list&error=file_missing');
    exit;
}
