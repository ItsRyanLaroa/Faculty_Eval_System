...
$rid = $_GET['rid'];
if(isset($_GET['fid']))
$staff_id = $_GET['fid']; // Changed from faculty_id to staff_id
if(isset($_GET['sid']))
$subject_id = $_GET['sid'];
$restriction = $conn->query("SELECT r.id,s.id as sid,st.id as staff_id,concat(st.firstname,' ',st.lastname) as staff,s.code,s.subject FROM staff_restriction_list r inner join staff_list st on st.id = r.staff_id inner join subject_list s on s.id = r.subject_id where academic_id ={$_SESSION['academic']['id']} and class_id = {$_SESSION['login_id']} and r.id not in (SELECT restriction_id from staff_evaluation_list where academic_id ={$_SESSION['academic']['id']} and student_id = {$_SESSION['login_id']} ) ");
?>
<style>
        ...
        <div class="col-md-3">
            <div class="list-group">
                ...
                while($row=$restriction->fetch_array()):
                    if(empty($rid)){
                            $rid = $row['id'];
                            $staff_id = $row['staff_id']; // Changed from faculty_id to staff_id
                            $subject_id = $row['sid'];
                        }
                ?>
                <a class="list-group-item list-group-item-action <?php echo isset($rid) && $rid == $row['id'] ? 'active' : '' ?>" href="./index.php?page=evaluate&rid=<?php echo $row['id'] ?>&sid=<?php echo $row['sid'] ?>&fid=<?php echo $row['staff_id'] ?>"><?php echo ucwords($row['staff']).' - ('.$row["code"].') '.$row['subject'] ?></a> <!-- Changed faculty to staff -->
            <?php endwhile; ?>
            </div>
        </div>    
        <div class="col-md-9">
            <div class="card card-outline card-info">
                ...
                <div class="card-body">
                    ...
                    <form id="manage-evaluation">
                        <input type="hidden" name="class_id" value="<?php echo $_SESSION['login_id'] ?>">
